        ...
        public function boot(): void
        {
            View::composer('*', function ($view) {
                if (Auth::check()) {
                    ...
                    // グループ情報
                    $groupIds = Cache::remember("$cacheKeyPrefix:group_ids", 60, function () use ($userId) {
                        return GroupMember::where('user_id', $userId)->pluck('group_id')->toArray();
                    });
                    $groups = Cache::remember("$cacheKeyPrefix:groups", 60, fn () =>
                        Group::whereIn('id', $groupIds)->get()
                    );
                    // 旅程情報
                    $itineraries = Cache::remember("$cacheKeyPrefix:itineraries", 60, fn () =>
