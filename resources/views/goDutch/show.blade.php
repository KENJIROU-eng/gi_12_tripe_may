                ...
                <div class="p-6 text-black dark:text-gray-100 h-full">
                    ...
                    <div class="relative flex items-center justify-center h-16 my-5">
                        ...
                        <div x-data="{ open: {{ $hasCreateError ? 'true' : 'false' }} }">
                            {{-- trigger --}}
                            <button @click="open = true" class="absolute right-40 inline-flex items-center px-3 py-1.5 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold rounded-lg shadow">
                    ...
                    <div class="mx-auto h-4/5 mt-6 overflow-hidden">
                        @if (count($all_bills) > 0)
                            ...
                            <div class="grid grid-cols-5 items-center text-center text-md mb-4 mt-6 gap-4 pb-4 border-b-2 border-t-2 border-green-500 py-2 font-bold">
                                {{-- user avatar --}}
                                    ...
                                    <div class="col-span-1 flex items-center justify-center gap-4">
                                        ...
                                        <div x-data="{ open: {{ $hasEditError ? 'true' : 'false' }} }">
                                            <button
                                                ...
                                                title="編集"
                                            >
                                                <i class="fa-solid fa-pen-to-square text-black text-lg"></i>
                                            </button>
                                        ...
                                        {{-- 削除フォーム --}}
                                        <form
                                            action="{{ route('goDutch.delete', ['itinerary_id' => $itinerary->id, 'bill_id' => $bill->id]) }}"
                                            method="post"
                                        >
                                            @csrf
                                            @method('DELETE')
                        ...
                        @if (count($all_bills) > 0)
                            @if (!empty($details))
                                ...
                                <div class="grid grid-cols-4 items-center text-center text-md mb-4 mt-6 gap-4 pb-4 border-b-2 border-t-2 border-green-500 py-2 font-bold">
                                    {{-- user avatar --}}
                                    <div class="col-span-1">
                                        {{-- user show --}}
                                        <p>PayUser name -> GetUser name</p>
                                    </div>
                                    <div class="col-span-1">
                                        <p>Price</p>
                                    </div>
                                ...
                                <div class="max-h-[160px] mt-2 overflow-y-auto">
                                        @foreach ($details as $detail)
                                            <div class="grid grid-cols-4 items-center text-center text-md my-4 gap-4 pb-4">
                                                ...
                                                <div class="col-span-1">
                                                    <span class="text-md text-gray-700 text-red-500">${{  $detail[2]  }}</span>
                                                ...
                                                <div class="col-span-1">
                                                    <span class="text-md text-blue-600">After you finalize the bills, you can use paypal for payment</span>
                                                </div>
                                            </div>
                                        @endforeach
                                        {{-- @forelse ($groupMembers as $groupMember) --}}
                                            {{-- <div class="grid grid-cols-4 items-center text-center text-md my-4 gap-4 pb-4"> --}}
                                                ...
                                                {{-- <div class="col-span-1 flex items-center justify-center space-x-4"> --}}
                                                    {{-- 左側ユーザー --}}
                                                    {{-- <div class="text-center">
                                                        <a href="{{ route('profile.show', $groupMember->id) }}" class="block w-10 h-10 rounded-full overflow-hidden mx-auto">
                                                        ...
                                                        <p class="text-sm mt-1">{{ $groupMember->name }}</p>
                                                    </div> --}}
                                                    {{-- 矢印 --}}
                                                    {{-- <div class="text-red-500 text-xl">
                                                        <i class="fa-solid fa-arrow-right"></i>
                                ...
                                <div class="flex justify-center mt-3">
                                    <a href="{{ route('goDutch.finalize', $itinerary->id) }}" class="bg-blue-500 text-white w-1/2 px-2 font-bold text-center rounded hover:bg-blue-600 max-w-md text-lg">
                                        finalize the bills
                                    </a>
                                </div>
                            @endif
