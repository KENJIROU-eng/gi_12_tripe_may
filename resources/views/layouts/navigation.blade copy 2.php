<nav x-data="{ open: false, planOpen: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 fixed top-0 left-0 right-0 z-40 shadow">
        ...
        <div class="flex justify-between h-16 items-center">
                        ...
                        <ul class="text-sm text-gray-700 dark:text-gray-200 space-y-1">
                            @foreach($todayItineraries as $itinerary)
                                <li class="grid grid-cols-[60px_1fr]">
                                    ...
                                    <span class="text-black">{{ \Carbon\Carbon::parse($itinerary->start_date)->format('M. d, Y') }}
                                ...
                                <li class="grid grid-cols-[60px_1fr]">
                                    ...
                                    <a href="{{ route('itinerary.show', $itinerary->id) }}" class="text-blue-500 hover:underline">
                                        {{ Str::limit($itinerary->title, 20) }}
                                    </a>
                                </li>
                                <li class="grid grid-cols-[60px_1fr]">
            ...
            @if ($groupIds)
                <div x-data="{ open: false }" class="relative">
                    <button @click.stop="open = !open" class="relative px-2 text-gray-600 dark:text-gray-200 hover:text-yellow-500">
                        <i class="fa-solid fa-bell text-lg"></i>
                        @if ($nonReadCount_total > 0)
                            <span class="absolute -top-1 -right-1 inline-block w-4 h-4 bg-red-500 text-white text-xs rounded-full flex items-center justify-center">{{ $nonReadCount_total }}</span>
                        @endif
                    </button>
                        ...
                        @foreach ($groups as $group)
                            @if ($nonReadCount[$group->id] > 0)
                                <a href="{{ route('message.show', $group->id) }}"
                                   class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-800">
                                    {{ $group->name }}
    ...
    <!-- モバイルメニュー -->
    <div x-show="open" @click.outside="open = false" x-cloak class="sm:hidden fixed inset-0 bg-black bg-opacity-50 z-30">
        <div class="w-64 bg-white dark:bg-gray-900 p-4 h-full overflow-y-auto">
            @auth
