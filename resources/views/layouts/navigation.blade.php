...
<nav x-data="{ open: false }"
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16 items-center">
            ...
            <div class="hidden sm:flex space-x-8">
                @if(Auth::check())
                    <x-nav-link :href="route('post.list')" :active="request()->routeIs('post*')" class="text-xs lg:text-lg">Post</x-nav-link>
                    <div class="border-x-2 border-gray-300 dark:border-gray-600 px-2">
                        <x-nav-link :href="route('itinerary.index')" :active="request()->routeIs('itinerary*')" class="text-xs lg:text-lg">Itinerary</x-nav-link>
                    </div>
                    <x-nav-link :href="route('groups.index')" :active="request()->routeIs('group*')" class="text-xs lg:text-lg">Group</x-nav-link>
                @endif
            </div>
    ...
    <div class="w-64 bg-white dark:bg-gray-900 p-4 overflow-y-auto relative">
        <!-- ✕ ボタン -->
        <button @click="open = false" class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 focus:outline-none">
