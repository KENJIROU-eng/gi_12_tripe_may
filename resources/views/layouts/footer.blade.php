<nav x-data="{ open: window.innerWidth >= 640, addCountry: false }"
    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-2">
        ...
        {{-- フッター本体 --}}
        <div :class="{'max-h-0 overflow-hidden opacity-0': !open && window.innerWidth < 640, 'opacity-100': open || window.innerWidth >= 640}"
            class="transition-all duration-300 ease-in-out flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-gray-600 dark:text-gray-300">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-2 sm:gap-6">
                ...
                {{-- コピーライト（常に表示） --}}
                <p class="text-xs sm:text-sm text-center sm:text-left w-full sm:w-auto">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-3 sm:gap-6 text-xs sm:text-sm">
                <form method="POST" action="{{ route('weather.setCountry') }}"
                    class="flex items-center gap-2 w-full sm:w-auto">
                    @csrf
                    <select name="country_id" onchange="this.form.submit()"
                        class="min-w-[10rem] w-full sm:w-60 border rounded px-2 py-1 text-sm">
            ...
            <div @click.away="addCountry = false" class="bg-white dark:bg-gray-900 p-6 rounded-lg shadow-xl w-full max-w-sm space-y-4">
                ...
                <form method="POST" action="{{ route('countries.store') }}" class="space-y-3">
                    ...
                    <div>
                        ...
                        <input name="city" required pattern="[A-Za-z\s]+" title="Please enter city names in English" placeholder="e.g. Tokyo" class="w-full px-3 py-2 border rounded dark:bg-gray-800 dark:text-white">
                    </div>
