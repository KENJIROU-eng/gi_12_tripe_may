                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg h-full">
                    <div class="p-4 text-black dark:text-gray-100 h-full"> --}}
                        ...
                        {{-- <div class="relative flex items-center justify-center h-8 my-2">
                            ...
                            <a href="{{ route('post.create') }}" class="ml-auto mr-3 right-40 no-underline text-end text-md sm:text-xl">
                        ...
                        {{-- <div class="mx-auto  mt-4 ">
                            ...
                            <div id="scroll-container" class="max-h-[730px] overflow-auto pb-4">
                                <div id="post-container" class="flex flex-wrap -mx-2" data-masonry='{"itemSelector": ".post-item", "columnWidth": ".post-sizer", "percentPosition": true }' wire:ignore>
                                    <div class="post-sizer w-full sm:w-1/2 lg:w-1/3"></div>
                                    @foreach ($all_posts as $post)
                                        <div class="post-item w-full sm:w-1/2 lg:w-1/3 px-2 mb-4 transition duration-300 ease-in-out transform hover:scale-[1.03] hover:-translate-y-1">
                                            <div class="relative group bg-white dark:bg-gray-700 shadow-xl  overflow-hidden">
                                                <a href="{{ route('post.show', $post->id) }}">
                                                    <img src="{{ $post->image }}" alt="{{ $post->title }}"
                                                        class="w-full h-auto object-cover transition-transform duration-300 group-hover:scale-100 max-h-[480px]">
                                                </a>
                                                <div class="absolute inset-0 bg-black bg-opacity-30 flex flex-col justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
                                                    ...
                                                    <div class="flex items-center px-3 pb-3 pointer-events-auto">
                                                        <a href="{{ route('profile.show', $post->user->id) }}">
                                                            @if ($post->user->avatar)
                                                                <img src="{{ $post->user->avatar }}" alt="{{ $post->user->name }}" class="object-cover rounded-full w-12 h-12 hover:scale-110 duration-300 transitio border border-white shadow-lg">
                                                            @else
...
{{-- Masonry.js CDN --}}
    ...
    <script>
        ...
        function safeMasonry() {
            initMasonry();
            // 念のため数百ミリ秒後にももう一度実行（Livewireの遅延対応）
                    ...
                    <div class="mx-auto mt-4">
                        <form action="{{ route('post.search') }}" method="get" class="w-full">
                            <div class="flex justify-center items-center mb-3 w-full">
                                ...
                                <button type="submit" class="block text-white px-4 bg-green-500 py-2 font-semibold hover:bg-green-600 transition rounded-md">Search</button>
                            </div>
                        </form>
                    </div>
